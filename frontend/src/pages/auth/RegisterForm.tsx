// Register form component for Financial Copilot
// User registration with email/password and optional name fields
...
const RegisterForm: React.FC<RegisterFormProps> = ({ onError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Note: This uses deprecated auth - RegisterForm is not used in main flow
      await register(); // Deprecated auth doesn't take parameters
      // Success - AuthContext will handle navigation
    } catch (error: any) {
      onError('Registration failed - please use SupabaseAuthPage instead');
    } finally {
      setIsLoading(false);
    }
  };
